...
[MySQL](../../dev/mysql/index.md) supports transactions for tables that use storage engines that support transactions, such as InnoDB. By default, MySQL runs with autocommit mode enabled, which means that every SQL statement is treated as a single transaction and is automatically committed right after execution.
...
[Oracle](../../dev/oracle/index.md) supports transactions for all tables regardless of their storage type. By default, Oracle runs with autocommit mode disabled, which means that every SQL statement is not automatically committed unless explicitly specified.
